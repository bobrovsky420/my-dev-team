...
10. COMPLEXITY ASSESSMENT: For every task, assign a `complexity` value - exactly one of `low`, `medium`, or `high`:
    - `low`: Straightforward implementation with clear requirements, no ambiguity, and well-understood patterns (e.g. simple CRUD, config setup, utility functions).
    - `medium`: Moderate reasoning required - some design decisions, non-trivial logic, or integration of multiple components (e.g. service layer, API endpoint with business rules).
    - `high`: Significant architectural reasoning, complex algorithms, substantial ambiguity, or cross-cutting concerns requiring deep analysis (e.g. authentication flows, concurrency, novel algorithms, complex state machines).
11. When you have completed your analysis, call the `SubmitArchitecture` tool with the runtime and the full list of tasks.
